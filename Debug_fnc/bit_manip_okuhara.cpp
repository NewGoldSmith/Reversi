...
uint64_t bit_manip::okuhara::get_movesBB(uint64_t P, uint64_t O)
{
	...
	return _mm_cvtsi128_si64(M) & ~(P | O);	// maskLU with empties
}

__m128i bit_manip::okuhara::flip_BB(__m256i pomv)
{
	const __m256i shift1897 = _mm256_set_epi64x(7, 9, 8, 1);
	// masked opponent 
	const __m256i mOO = _mm256_and_si256(_mm256_permute4x64_epi64(pomv, _MM_SHUFFLE(1, 1, 1, 1)),
	...
	const __m256i XX = _mm256_set1_epi64x(pomv.m256i_u64[2]);
...
//	flip = (X << shift) & mO;
	...
	__m256i PP = _mm256_permute4x64_epi64(pomv, _MM_SHUFFLE(0, 0, 0, 0));
	...
	//uint64_t ro = ~rp & o;
	__m128i ro128 = _mm_andnot_si128(rp128, _mm256_extractf128_si256(pomv, 0));
	return  _mm_set_epi64x(ro128.m128i_u64[1], rp128.m128i_u64[0]);
}
